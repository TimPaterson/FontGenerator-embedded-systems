...
public partial class MainWindow : Window
{
	...
	public static Brush TableLabelBrush => CharTableLabelBrush;
	public uint? ProjectVersion
	{
		get => m_project.Version;
		set => m_project.Version = value;
	}

	public bool HasVersion
	{
		get => ProjectVersion != null;
		set
		{
			if (value)
			{
				if (uint.TryParse(txtVersion.Text, out uint version))
					ProjectVersion = version;
				else
					ProjectVersion = 0;

				txtVersion.Text = ProjectVersion.ToString();
			}
			else
				ProjectVersion = null;
		}
	...
	{
		get => m_project.Signature;
		set => m_project.Signature = value;
	}
	...
	void LoadProject(string fileName)
	{
		...
		chkVersion.IsChecked = HasVersion;
		txtVersion.Text = ProjectVersion?.ToString();
	...
	private void btnGenerate_Click(object sender, RoutedEventArgs e)
	{
		...
		if (chkVersion.IsChecked == true && ProjectVersion != null)
			output.WriteVersion((uint)ProjectVersion, Signature);
		...
		foreach (FontInfo fontInfo in m_project.FontInfos)
		{
			if (fontInfo.Name != StrEmptyItem)
			{
				...
				if (m_project.Stride16Bit)
				{
					...
					{
						(fontBits.arPx[i + 1], fontBits.arPx[i]) = (fontBits.arPx[i], fontBits.arPx[i + 1]);
					}
				}
